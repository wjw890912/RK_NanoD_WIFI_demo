...
__attribute__((section("HeapStack"))) __align(8)
...
//char  test_buffer[1024] __attribute__((at((uint32_t)0xD0000000)));
...
#define MEMORY_CHECK_MAX 3000
size_t memory_check_buf[MEMORY_CHECK_MAX] = {0};
char * memory_check_task_buf[MEMORY_CHECK_MAX] = {0};
int memory_check_flag =0;
void memory_leak_check_start()
{
    int i;
    vTaskSuspendAll();
    memory_check_flag = 1;
    for(i=0;i<MEMORY_CHECK_MAX;i++)
    {
        ...
        {
            printf("unfree memory is 0x%04x task = %s\r\n",memory_check_buf[i],memory_check_task_buf[i]);

        }
    }
    memset(memory_check_buf,0,MEMORY_CHECK_MAX*4);
    xTaskResumeAll();
}

void memory_leak_check_watch()
{
    int i;
    for(i=0;i<MEMORY_CHECK_MAX;i++)
    {
        if(memory_check_buf[i] != 0)
        {
            unsigned char *puc = ( unsigned char * ) memory_check_buf[i];
            puc -= heapSTRUCT_SIZE;
            xBlockLink *pxLink = (xBlockLink *)puc;
            printf("unfree memory is 0x%04x , block size = %d, task = %s\r\n",
                memory_check_buf[i], pxLink->xBlockSize,memory_check_task_buf[i]);
        }
    }
}

void memory_leak_check_stop()
{
    memory_check_flag = 0;
}

void memory_leak_check_reset()
{
    memset(memory_check_buf,0,MEMORY_CHECK_MAX*4);
}
#endif
    ...
    if(memory_check_flag)
    {
        ...
        for(i=0;i<MEMORY_CHECK_MAX;i++)
        {
            if(memory_check_buf[i] == 0)
            {
                ...
                memory_check_task_buf[i] = pxCurrentTCB->pcTaskName;
                   break;
            }
