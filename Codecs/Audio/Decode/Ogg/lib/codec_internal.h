...
#if 1
typedef struct{
  floor1class  class[5];			/* [VIF_CLASS] maxclass+1*/
  char		   partitionclass[32]; /* [VIF_PARTS=info->partitions]; 0 to 15 */
  ogg_uint16_t postlist[65];		/* [VIF_POSIT+2]; first two implicit */
  char		   forward_index[65];	/* [VIF_POSIT+2]; */
  char		   hineighbor[63]; 	/* [VIF_POSIT]; */// 63
  char		   loneighbor[63]; 	/* [VIF_POSIT]; */
...
typedef struct vorbis_info_mapping{
 ...
 // submap        *submaplist;
  unsigned char  chmuxlist[2];
   submap        submaplist[2];
  int            coupling_steps;
  //coupling_step *coupling;
  coupling_step coupling[2];
} vorbis_info_mapping;
...
#endif
#if 0
  vorbis_info_mode		 mode_param[2];
  vorbis_info_mapping	 map_param[2];
  char					 floor_type[2];
  vorbis_info_floor 	*floor_param[2];
  vorbis_info_residue	 residue_param[2];
  codebook				 book_param[44];
#endif
