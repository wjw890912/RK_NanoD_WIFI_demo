...
typedef struct ogg_buffer {
  //unsigned char      *data;
  unsigned char      data[1024];  //与 CHUNKSIZE 有关
  long                size;
  int                 refcount;
...
typedef struct {
  ...
  long           bytes;
  long           b_o_s;//表示该包是否为一逻辑流开始的标识符，1 是
  long           e_o_s;//表示该包是否为一逻辑流结束的标识符1 是
  ogg_int64_t    granulepos;
  ogg_int64_t    packetno;     /* sequence number for decode; the framing
                                  knows where there's a hole in the data,
                                  but we need coupling so that the codec
...
/* Ogg BITSTREAM PRIMITIVES: bitstream ************************/
 ...
 __inline void  oggpack_readinit(oggpack_buffer *b,ogg_reference *r);
extern  __inline  long  oggpack_look(oggpack_buffer *b,int bits);
extern __inline  void  oggpack_adv(oggpack_buffer *b,int bits);
extern __inline  long  oggpack_read(oggpack_buffer *b,int bits);
extern __inline  long  oggpack_bytes(oggpack_buffer *b);
extern __inline  long  oggpack_bits(oggpack_buffer *b);
extern __inline  int   oggpack_eop(oggpack_buffer *b);
