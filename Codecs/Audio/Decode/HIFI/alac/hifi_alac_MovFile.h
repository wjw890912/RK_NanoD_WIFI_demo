...
extern FILE * fopen_hl_h(const char * path,const char * mode);

extern unsigned short  fread_hl_h(void*buffer,size_t size,size_t count,FILE*stream);
extern unsigned char fseek_hl_h(FILE *stream, long offset, int fromwhere);
extern unsigned  long ftell_hl_h(FILE *stream);
extern unsigned char  fclose_hl_h(FILE *stream);
...
#else
#define MovRead     fread_hl_h
#define MovSeek     fseek_hl_h
#define MovOpen     fopen_hl_h
#define MovClose    fclose_hl_h
#define MovTell     ftell_hl_h
#define MovWrite    fwrite
#define MovFILE     FILE
#endif

#define MovSTopen   movBufInit_h
#define MovSTclose  movBufdeinit_h
#define MovSTread   movBufRead_h
#define MovSTseek   movBufSeek_h
#define MovSTtell   movBufTell_h
#define MOV_ST    	MovBuf
#define MALLOC      MovMemAlloc_h
#define FREE        MovMemFree_h
...
#define SIGN_ALAC 0x63616C61 //alac
...
#define AUDIOCODE_ALAC 0x63616C61 //alac
...
EXT MovFileinf gMovFile_h;
EXT SKIPSTATE gBeingSkip_h;


EXT int MovFileInit_h(FILE* alac_raw_file);

EXT int	MovFileParsing_h(MOV_ST *videoFile);

EXT void MovFileClose_h(void);
...
EXT void MovSynAudio2Video_h(long videoSampleNo);
...
EXT void MovGenerateMovFrmHeader_h(char *pHeader,
								 uint32 frmLength,
								 uint32 msTime);
...
EXT int MovIF_SynAudio2Video_h(unsigned int timems);

EXT unsigned int MovIF_AudioGetCurrentTime_h(void);
