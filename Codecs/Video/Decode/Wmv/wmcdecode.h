...
#ifdef __arm                // turn off DRM stuff for now
#define WMCAPI_NO_DRM
#endif

#define NONSDMI_RIGHT   0x02
#define BURNCD_RIGHT    0x08
#define SDMI_RIGHT      0x10
...
#ifndef _WMCDATEPARAMS_DEFINED
#define _WMCDATEPARAMS_DEFINED
typedef  struct tagWMCDateParams
{
    U16_WMC year;
    U8_WMC  month;
    U8_WMC  day;
 ...
 * Syntax
 * ------
 *   tWMCDecStatus WMCDecInit (HWMCDECODER* phWMCDec, tVideoFormat_WMC tVideoOutputFormat, Bool_WMC bDiscardIndex,I32_WMC iPostProcLevel, Bool_WMC bIsNoBufferMode);
 *
 *
 * where:
 *
 ...
 *                          1 to 4 : Different level of Post Filter
 *                          1: Lowest level of Post Filter. Quality is poor. Decoding is fast.
 *                          4: Heigest level of Post Filter. Best quality. Decoding is slow.
 *   bIsNoBufferMode        TRUE_WMC if user wants output without buffering compressed data
 *                          This mode can only be used for local decode. File I/O are more in this case
 *                          FALSE_WMC if buffer mode is used.
 *
 * Return Value
 * ------------
 ...
 * Syntax
 * ------
 *      tWMCDecStatus WMCFileLicenseInit (HWMCDECODER hWMCDec,
 *                                        tWMCDecFileLicParams * plic,
 *                                        U16_WMC CheckLicTypes,
 *                                        tWMCDateParams currentDate);
 * where:
 *
 *      hWMCDec                 Decoder object HWMCDECODER
 *      plic                    point to license
 *      right                   request right
 *      currentDate             current date;

*/
tWMCDecStatus WMCFileLicenseInit (HWMCDECODER hWMCDec, tWMCDecFileLicParams * plic, U16_WMC right, tWMCDateParams currentDate);
 ...
 * ------------
 *   Error code.
 *	 WMCDec_Succeeded if succeeds.
 *   WMCDec_InValidArguments if arguments are invalid
 *   WMCDec_InvalidStreamId if nStreamNum is not Audio
 *
 */
 ...
 *	 WMCDec_Succeeded if succeeds.
 *   WMCDec_InValidArguments if arguments are invalid
 *   WMCDec_InvalidStreamId if nStreamNum is not Video
 *
 *
 */
 ...
 *	 WMCDec_Succeeded if succeeds.
 *   WMCDec_InValidArguments if arguments are invalid
 *   WMCDec_InvalidStreamId if nStreamNum is not Binary
 *
 */
 ...
 *
 *  iHurryUp                    It is used for Video. -1 is ignore, 0..4 sets post processing level
 *                              iHurryUp < -1 indicates number of ms player is behind and can cause decode to be faster and lower quality.
 * Return Value
 * ------------
 ...
 *   ptPresentationTime TimeStamp for the output. This Parameter can be NULL if user doesn't want time stamp.
 *
 * Return Value
 * ------------
 *   WMCDec_Succeeded           OK
...
tWMCDecStatus WMCDecIsVideoFirstFrameADuplication (HWMCDECODER hWMCDec, Bool_WMC * pbIsFirstFrameADuplication);

tWMCDecStatus WMCDecGetVideoOutputLastFlush (HWMCDECODER hWMCDec, U8_WMC *pu8VideoOutBuffer, U32_WMC u32VideoOutBufferSize, U32_WMC * pu32OutDataSize, I64_WMC* ptPresentationTime, Bool_WMC *bIsKeyFrame, tWMCDecodeDispRotateDegree tDispRotateDegree);
 ...
 * Syntax
 * ------
 *   tWMCDecStatus WMCDecSkipToNextKey (HWMCDECODER hWMCDec);
 *
 * where:
 *
 *   hWMCDec                    Decoder object HWMCDECODER.
 ...
 * Description
 * -----------
 * Sets the mask which enables individual callback from the WMVA codec.
 *
 *
 *
 * Syntax
 * ------
 *   tWMCDecStatus WMCDecEnableMask (HWMCDECODER* phWMCDec, U32_WMC u32EnableMask );
 *
 * where:
 *
 *   hWMCDec                    Decoder object HWMCDECODER.
 *   u32EnableMask              "or" of bits to enable individual callbacks.  See wmvdec_api.h for details.
 *
 * Return Value
 * ------------
 *   WMCDec_Succeeded           OK
