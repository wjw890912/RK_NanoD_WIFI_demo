...
/*******************************************************************************
** Name: SDC_SetChannel
** Input:SDMMC_PORT SDCPort, uint32 Channel
** Return: rk_err_t
** Owner:Aaron.sun
** Date: 2014.6.20
** Time: 15:35:37
*******************************************************************************/
READ API rk_err_t SDC_SetChannel(SDMMC_PORT SDCPort, uint32 Channel)
{
    if(Channel == 1)
    {
        Grf->IOMUX_CON1 = 1<<0 | 1<<16; //sdmmc_xxx_p1 available for sdmmc
    }
    else
    {
        Grf->IOMUX_CON1 = 0<<0 | 1 << 16; //sdmmc_xxx_po
    }
}
...
** Time: 16:51:42
*******************************************************************************/
READ API void SDC_DisableDMA(SDMMC_PORT SDCPort)
{
...
** Time: 16:50:45
*******************************************************************************/
READ API void SDC_EnableDMA(SDMMC_PORT SDCPort)
{
     SDC_REG_T *      pReg = SDMMC(SDCPort);
     pReg->SDMMC_CTRL |= ENABLE_DMA;
}
...
** Time: 15:26:46
*******************************************************************************/
READ API uint32 SDC_GetFIFOAddr(SDMMC_PORT SDCPort)
{
...
rk_err_t SDC_Init(SDMMC_PORT SDCPort, uint32 FreqKHz)
{
    ...
    value |= (SBE_INT | EBE_INT | FRUN_INT | DRTO_INT | RTO_INT | DCRC_INT | RCRC_INT | RE_INT);
...
** Time: 14:22:57
*******************************************************************************/
void SDC_DevReset(eGPIO_CHANNEL gpioPort, eGPIOPinNum_t GpioNum, BOOL DevReset)
{
    if(DevReset)
    {
        Gpio_SetPinLevel(gpioPort, GpioNum, GPIO_LOW);
    }
    else
    {
        Gpio_SetPinLevel(gpioPort, GpioNum, GPIO_HIGH);
    }
}
...
** Time: 14:20:42
*******************************************************************************/
void SDC_DevPower(eGPIO_CHANNEL gpioPort,eGPIOPinNum_t GpioNum, BOOL DevPower)
{
    if(DevPower)
    {
        Gpio_SetPinLevel(gpioPort, GpioNum, GPIO_HIGH);
