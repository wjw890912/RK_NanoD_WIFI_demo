...
READ API void mp3_wait_synth(void)
{
    long synth_e_cnt = 0;
    while (is_synthing==1)
    {
...
*******************************************************************************/
_CPU_NANOC_LIB_HW_MP3_SYN_READ_
READ API void mad_synth_fouth_frame(struct mad_frame *frame, int whichch,int ns)
{
...
*******************************************************************************/
_CPU_NANOC_LIB_HW_MP3_SYN_READ_
READ API void synth_handler(void)
{
    SYNTH_EOIT_REG = 0;
...
*******************************************************************************/
_CPU_NANOC_LIB_HW_MP3_SYN_READ_
READ FUN void mad_synth_mute(struct mad_synth *synth)
{
    unsigned int ch, s, v;
...
*******************************************************************************/
_CPU_NANOC_LIB_HW_MP3_SYN_READ_
READ FUN void synth_output(void)
{
    int i;
    short *start_pos;
...
*******************************************************************************/
_CPU_NANOC_LIB_HW_MP3_SYN_READ_
READ FUN void synth_fouth_frame_hw(struct mad_frame * frame, mad_fixed_t (fouth_frame_sbsample)[36][32],
...
*******************************************************************************/
_CPU_NANOC_LIB_HW_MP3_SYN_READ_
READ FUN void start_next_synth(void)
{
    if (synth_hw_s < synth_hw_ns)
    {
        SYNTH_CNFG_REG = (((15 - synth_hw_phase)<<1)|synth_hw_ch);      //config the phase & channel;
...
*******************************************************************************/
_CPU_NANOC_LIB_HW_MP3_SYN_READ_
READ FUN int DMA1ToSynth(unsigned long pSrc)
{
    int tmp;
    ...
    CLRENA0_REG = tmp;//0xfffffffe;   //关中断
    ...
    SETENA0_REG = tmp;//0xfffffffe;//开中断
...
*******************************************************************************/
_CPU_NANOC_LIB_HW_MP3_SYN_READ_
READ FUN int DMA1FromSynth(unsigned long pDst)
{
    int tmp;
...
char test_data[] = {0x00};
int test_out_data_r[] = {0x00};

SHELL API rk_err_t hw_syn_shell()
{
    int whichch = 0;
    int ns = 0;
    mad_frame *frame =(mad_frame *)test_data;
    Hw_syn_Init();
