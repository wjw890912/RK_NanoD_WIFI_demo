...
extern UINT32 Image$$MAIN_STACK$$RW$$Limit;


__attribute__((section("MainStack"))) __align(4) uint32 MainStack[128];
__attribute__((section("IdleStack"))) __align(4) uint32 IdleStack[128];
/*
--------------------------------------------------------------------------------
  Function name :
  Author        : ZHengYongzhi
...
ExecFunPtr exceptions_table[NUM_INTERRUPTS] = {
    ...
    (ExecFunPtr)(&Image$$MAIN_STACK$$RW$$Limit),
...
//    ASSERT(((UINT32)exceptions_table & 0x000003ff) == 0);
    ...
    if(nvic->VectorTableOffset != (UINT32)exceptions_table)
    {
        ...
        exceptions_table[0] = (ExecFunPtr)&Image$$MAIN_STACK$$RW$$Limit;
