...
*******************************************************************************/
_LIB_ROCKCODEC_COMMON_
COMMON API void Codec_PowerDown(void)
{
...
void ACodec_Set_DAC_Mode(CodecOut_sel_t OUT_Mode,CodecFS_en_t CodecFS)
{
    ...
    //rk_printf("ACODEC_VCTL = 0x%x",ReadAcodecReg(ACODEC_BASE+ACODEC_VCTL));
    config = 0;
    ...
    //rk_printf("ACODEC_DACSR = 0x%x",ReadAcodecReg(ACODEC_BASE+ACODEC_DACSR));
    ...
    //rk_printf("ACODEC_CLKE = 0x%x",ReadAcodecReg(ACODEC_BASE+ACODEC_CLKE));

    //enbale i2s_rx
    MaskAcodecRegBits(ACODEC_BASE+ACODEC_DIGEN,I2SRX_EN_DISABLE,I2SRX_EN_ENABLE);
    //rk_printf("ACODEC_DIGEN = 0x%x",ReadAcodecReg(ACODEC_BASE+ACODEC_DIGEN));
    ...
    //rk_printf("ACODEC_DACPOPD = 0x%x",ReadAcodecReg(ACODEC_BASE+ACODEC_DACPOPD));

    MaskAcodecRegBits(ACODEC_BASE+ACODEC_HPLOCFG1,(0x1 << 0),0x1);
    //rk_printf("ACODEC_HPLOCFG1 = 0x%x",ReadAcodecReg(ACODEC_BASE+ACODEC_HPLOCFG1));

    MaskAcodecRegBits(ACODEC_BASE+ACODEC_RTCFG0,(0xff << 0),0x1);
    //rk_printf("ACODEC_RTCFG0 = 0x%x",ReadAcodecReg(ACODEC_BASE+ACODEC_RTCFG0));
    DelayMs(1);

    MaskAcodecRegBits(ACODEC_BASE+ACODEC_RTCFG1,(0x7 << 0),0x1);
    //rk_printf("ACODEC_RTCFG1 = 0x%x",ReadAcodecReg(ACODEC_BASE+ACODEC_RTCFG1));
    DelayMs(1);
...
void ACodec_ReadReg_printf(uint32 regaddr)
{
    printf("reg data = %d",ReadAcodecReg(regaddr));
}
...
void ACodec_dac_lineout_init()
{
    ...
    if(Codec_Power_Status == Codec_Power_null)
    {
        ...
        ACodec_Set_I2S_Mode(TFS_TX_I2S_MODE,ACodec_I2S_DATA_WIDTH16,IBM_TX_BUS_MODE_NORMAL,I2S_MST_MASTER);
        Codec_SetSampleRate(CodecFS_8000Hz);
        Codec_SetMode(Codec_DACoutHP,CodecFS_8000Hz);
        DelayMs(50);
