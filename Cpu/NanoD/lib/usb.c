...
{
    uint32 *address1 = (uint32 *)0x60080b24;
    uint32 *address0 = (uint32 *)0x60080e00;
    uint32 intr_status;
    uint32 intr_type;
    USB_OTG_REG *  otg_core = (USB_OTG_REG * )USB_REG_BASE;
...
COMMON API int32 HostStartWaitXfer(USB_REQ *urb, int32 timeout, UHC_CHN_INFO * pUsbHC)
{
    ...
    return (urb->status<0)? urb->status : hc->XferCnt;
}
...
COMMON API uint32 UsbGetHostIntType(UHC_CHN_INFO * pUsbHC)
{
    ...
    {
    	if(pUsbHC->EpType == DWC_OTG_EP_TYPE_INTR || pUsbHC->urb->pDev->DevDesc.bDeviceClass == 0xff)
    	{
...
COMMON FUN int32 HostProcessXfer(UHC_CHN_INFO * pUsbHC)
{
    ...
    else
    {
        ...
        if (!hc->XferStarted)
        {
        	//printf("HostStartXfer\r\n");
            HostStartXfer(hc);
            ret = 1;
        }
        else
        {
        	//printf("HostContinueXfer\r\n");
            ret = HostContinueXfer(hc);
        }
    }
OUT:
...
COMMON FUN static void HostStallIsr(UHC_CHN_INFO* hc)
{
    ...
    if (hc->EpType == DWC_OTG_EP_TYPE_BULK || hc->EpType == DWC_OTG_EP_TYPE_INTR)
    {
        /*
...
COMMON FUN static void HostNakIsr(UHC_CHN_INFO* hc)
{
    ...
    switch (hc->EpType) /** 0: Control, 1: Isoc, 2: Bulk, 3: Intr */
    {
        case DWC_OTG_EP_TYPE_CONTROL:
            ...
            break;			
        case DWC_OTG_EP_TYPE_BULK:
			//printf("bDeviceClass:%02x\r\n",urb->pDev->DevDesc.bDeviceClass);
			if(urb->pDev->DevDesc.bDeviceClass == 0xff)
			{
				HostUpdateXfer(hc, urb, HC_XFER_NAK);
				HostSaveToggle(hc, urb);
				HostHaltChn(hc, HC_XFER_NAK);
            ...
            break;
		case DWC_OTG_EP_TYPE_INTR:
			//printf("DWC_OTG_EP_TYPE_INTR NAK......\r\n");
            HostUpdateXfer(hc, urb, HC_XFER_NAK);
            HostSaveToggle(hc, urb);
            HostHaltChn(hc, HC_XFER_NAK);
			HostCompleteUrb(urb, -HC_XFER_NAK);
