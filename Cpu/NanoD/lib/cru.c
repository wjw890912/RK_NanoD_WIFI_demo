...
uint32 SetPllFreq(uint32 nHz, PLL_ARG_t *pPllArg)
{
    ...
    if (nHz<= 24000000)
    {
        ...
        CRU->CRU_CLKSEL_CON[1] = ((CLK_SEL_MASK_3 << 16) | (pclk_logic_div - 1)) << 8;
...
uint32 SetI2SClkOutFreq(I2S_output_clk_Sel Clk_Source)
{
    ...
    else if(Clk_Source == I2S1_CLK)
    {
        ...
        chip_freq.i2s_clk_out = chip_freq.i2s_clk[I2S_CH1];
    }
   ...
   UINT32 RealFreq, Source_clk,div_num,ret;
    ...
    if(I2sId == I2S_CH1)
    {
        if(Clk_Source == PLL_MUX_CLK)
        {
            ...
            chip_freq.i2s_clk[I2S_CH1] = Source_clk/(div_num + 1);
        ...
        else if(Clk_Source == I2S_XIN12M)
        {
            ...
            chip_freq.i2s_clk[I2S_CH1] = Source_clk;
        }

        return(chip_freq.i2s_clk[I2S_CH1]);
    }
    else if(I2sId == I2S_CH0)
    {
        if(Clk_Source == PLL_MUX_CLK)
        {
            Source_clk = chip_freq.armclk;
            ...
            chip_freq.i2s_clk[I2S_CH0] = Source_clk/(div_num + 1);
        }
        else if(Clk_Source == CLK_ACODEC_PLL)
        {
            ...
            chip_freq.i2s_clk[I2S_CH0] = (Source_clk/(div_num + 1))/20;
        ...
        else if(Clk_Source == I2S_XIN12M)
        {
            ...
            chip_freq.i2s_clk[I2S_CH0] = Source_clk;
        }

        return(chip_freq.i2s_clk[I2S_CH0]);
    }
...
/*
--------------------------------------------------------------------------------
  Function name : SetSdmmcFreq()
  Author        :
  Description   : set apb clock para

  Input         : Clock_Source_Sel Clk_Source,UINT32 TargetFreqHz

  Return        : chip_freq.sdmmc_clk

  desc:         : TargetFreqHz unit Hz
--------------------------------------------------------------------------------
*/
uint32 SetSdmmcFreq(Clock_Source_Sel Clk_Source, UINT32 TargetFreqHz)
{
    UINT32 RealFreq, Source_clk,div_num,ret;
    ...
    if(Clk_Source == PLL_MUX_CLK)
    {
        ...
        chip_freq.sdmmc_clk = Source_clk/(div_num + 1);
...
--------------------------------------------------------------------------------
*/
uint32 SetSdioFreq(Clock_Source_Sel Clk_Source,UINT32 TargetFreqHz)
{
    UINT32 RealFreq, Source_clk,div_num,ret;
    ...
    if(Clk_Source == PLL_MUX_CLK)
    {
        ...
        chip_freq.sdio_clk = Source_clk/(div_num + 1);
    ...
    return(chip_freq.sdio_clk);

}

/*
--------------------------------------------------------------------------------
  Function name : UINT32 GetSdioFreq(void)
  Author        :
  Description   :
