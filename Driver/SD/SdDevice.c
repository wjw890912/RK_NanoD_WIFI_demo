...
*******************************************************************************/
_DRIVER_SD_SDDEVICE_COMMON_
COMMON FUN rk_err_t SdDevResume(HDC dev)
{
    //SdDev Resume...
    SD_DEVICE_CLASS * pstSdDev =  (SD_DEVICE_CLASS *)dev;
    if (pstSdDev == NULL)
...
*******************************************************************************/
_DRIVER_SD_SDDEVICE_COMMON_
COMMON FUN rk_err_t SdDevSuspend(HDC dev)
{
    //SdDev Suspend...
    SD_DEVICE_CLASS * pstSdDev =  (SD_DEVICE_CLASS *)dev;
    if (pstSdDev == NULL)
...
INIT API rk_err_t SdDev_Delete(uint32 DevID, void * arg)
{
    ...
    if (gpstSdDevISR[pstSdDev->stSdDevice.DevID] == NULL)
    {
        return RK_ERROR;
    }

    //SdDev deinit...
    SdDevDeInit(gpstSdDevISR[pstSdDev->stSdDevice.DevID]);

    //Free SdDev memory...
    rkos_semaphore_delete(gpstSdDevISR[pstSdDev->stSdDevice.DevID]->osSdOperReqSem);
    rkos_memory_free(gpstSdDevISR[pstSdDev->stSdDevice.DevID]);

    //Delete SdDev...
    gpstSdDevISR[pstSdDev->stSdDevice.DevID] = NULL;

    //Delete SdDev Read and Write Module...
#ifdef __SYSTEM_FWANALYSIS_FWANALYSIS_C__
    FW_RemoveModule(MODULE_ID_SD_DEV);
#endif
#endif
...
INIT API HDC SdDev_Create(uint32 DevID, void * arg)
{
    ...
    //module overlay...
#ifdef __SYSTEM_FWANALYSIS_FWANALYSIS_C__
    FW_ModuleOverlay(MODULE_ID_SD_DEV, MODULE_OVERLAY_CODE);
#endif
    //device init...
