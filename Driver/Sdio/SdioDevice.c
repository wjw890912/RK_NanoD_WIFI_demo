...
INIT API rk_err_t SdioDevDelete(uint32 DevID ,void * arg)
{
    ...
    gpstSdioDevISR[DevID]->hSdc = NULL;
    rk_printf("delete osSdioOperReqSem\n");
...
INIT API HDC SdioDev_Create(uint32 DevID, void * arg)
{
...
#ifdef __SYSTEM_FWANALYSIS_FWANALYSIS_C__
    FW_ModuleOverlay(MODULE_ID_SDIO_DEV, MODULE_OVERLAY_CODE);
#endif

    printf("SdioDevInit \n");
    if (SdioDevInit(pstSdioDev) != RK_SUCCESS)
    {
        printf("SdioDevInit != RK_SUCCESS\n");
        rkos_semaphore_delete(pstSdioDev->osSdioOperReqSem);
        rkos_memory_free(pstSdioDev);
        return (HDC) RK_ERROR;
    }
    ...
    if (gSdioDeviceHost->devCnt == 0)
    {
        ...
        RKTaskCreate(TASK_ID_SDIO_IRQ_TASK,0,NULL);
    }
...
*******************************************************************************/
_DRIVER_SDIO_SDIODEVICE_INIT_
INIT FUN rk_err_t cistpl_funce(SDIO_DEVICE_CLASS * pstSdioDev, SDIO_FUNC *func, const uint8 *buf, uint32 size)
{
    if (size < 1)
        return -EINVAL;
...
*******************************************************************************/
_DRIVER_SDIO_SDIODEVICE_INIT_
INIT FUN rk_err_t cistpl_manfid(SDIO_DEVICE_CLASS * pstSdioDev, SDIO_FUNC *func, const uint8 *buf, uint32 size)
{
...
INIT FUN rk_err_t SdioDevDeInit(SDIO_DEVICE_CLASS * pstSdioDev)
{
    if (gSdioDeviceHost->devCnt)
    {
        ...
        if (gSdioDeviceHost->devCnt == 0)
        {
            if (gSdioDeviceHost->SdioIrqTaskHandle)
                gSdioDeviceHost->SdioIrqTaskHandle = NULL;
            ...
            //vTaskDelete(gSdioDeviceHost->SdioIrqTaskHandle);
