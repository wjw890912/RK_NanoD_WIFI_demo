...
typedef  struct _USBOTG_DEVICE_CLASS
{
	...
	pSemaphore osUsbOtgOperLockSem;
...
COMMON API int32 HostBulkMsg(HOST_DEV *pDev, uint32 pipe, void * data, int len, int timeout)
{
	...
	rkos_semaphore_take(gpstUsbOtgDevISR[0]->osUsbOtgOperLockSem, MAX_DELAY);	
    urb = HostAllocUrb();
    ...
    HostFreeUrb(urb);
	rkos_semaphore_give(gpstUsbOtgDevISR[0]->osUsbOtgOperLockSem);
    return ret;
}

_DRIVER_USB_USBOTGDEV_COMMON_
COMMON API int32 HostIntMsg(HOST_DEV *pDev, uint32 pipe, void * data, int len, int timeout)
{
    int32 ret;
    USB_REQ * urb = NULL;
    uint8 epdir = USBPipeDir(pipe);
...
*******************************************************************************/
_DRIVER_USB_USBOTGDEV_COMMON_
COMMON FUN int32 HostGetDesc(HOST_DEV *pHDev, uint8 type, uint8 index, void *buf, int size)
{
    int32 i, ret;
...
*******************************************************************************/
_DRIVER_USB_USBOTGDEV_COMMON_
COMMON FUN void HostEp0ReInit(HOST_DEV *pHDev, uint16 wMaxPacketSize)
{
...
*******************************************************************************/
_DRIVER_USB_USBOTGDEV_COMMON_
COMMON FUN int32 HostSetAddr(HOST_DEV *pHDev)
{
    int32 ret, i;
    uint8 devnum = 1;
...
*******************************************************************************/
_DRIVER_USB_USBOTGDEV_COMMON_
COMMON FUN int32 HostNewDev(HOST_DEV *pHDev)
{
    int32 ret = -HOST_ERR;
    uint16 length;
    uint32 buf[256/4];
...
INIT API rk_err_t UsbOtgDev_Delete(uint32 DevID ,void * arg)
{
	...
	rkos_semaphore_delete(gpstUsbOtgDevISR[DevID]->osUsbOtgOperLockSem);
	...
	rkos_memory_free(gpstUsbOtgDevISR[DevID]->pHDev);
...
INIT API HDC UsbOtgDev_Create(uint32 DevID, void * arg)
{
	...
	phost_dev = rkos_memory_malloc(sizeof(HOST_DEV));
	if(phost_dev == NULL)
	{
		rkos_memory_free(pstUsbOtgDev);
		return NULL;
	}
	memset(phost_dev, 0, sizeof(HOST_DEV));
	pstUsbOtgDev->pHDev = phost_dev;

    //init handler...
    pstUsbOtgDev->osUsbOtgOperSem  = rkos_semaphore_create(1,0);
	pstUsbOtgDev->osUsbOtgOperLockSem  = rkos_semaphore_create(1,1);
    ...
    if (((pstUsbOtgDev->osUsbOtgOperSem) == 0)
			|| ((pstUsbOtgDev->osUsbOtgOperLockSem) == 0)
            ...
            || ((pstUsbOtgDev->osUsbOtgEpSem[2]) == 0))
    {
		...
		rkos_semaphore_delete(pstUsbOtgDev->osUsbOtgOperLockSem);
		...
		rkos_memory_free(pstUsbOtgDev->pHDev);
        rkos_memory_free(pstUsbOtgDev);
        return (HDC) RK_ERROR;
    }
