...
struct wpa_driver_ops {
	 ...
	 * associated.
	 */
	int (*get_bssid)(void *priv, uint8 *bssid);
	 ...
	 * string.
	 */
	int (*get_ssid)(void *priv, uint8 *ssid);
	 ...
	 */
	int (*set_key)(void *priv, wpa_alg alg, const uint8 *addr,
		       int key_idx, int set_tx, const uint8 *seq, size_t seq_len,
	 ...
	 */
	int (*scan)(void *priv, const uint8 *ssid, size_t ssid_len);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*deauthenticate)(void *priv, const uint8 *addr, int reason_code);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*disassociate)(void *priv, const uint8 *addr, int reason_code);
	 ...
	 */
	int (*add_pmkid)(void *priv, const uint8 *bssid, const uint8 *pmkid);
	 ...
	 */
	int (*remove_pmkid)(void *priv, const uint8 *bssid, const uint8 *pmkid);
	 ...
	 * a MAC address. */
	const uint8 * (*get_mac_addr)(void *priv);
	 ...
	 */
	int (*send_eapol)(void *priv, const uint8 *dest, uint16 proto,
	 ...
	 */
	int (*mlme_setprotection)(void *priv, const uint8 *addr, int protect_type,
				  int key_type);
	 ...
	 * (management frame processing) to wpa_supplicant.
	 */
	int (*set_ssid)(void *priv, const uint8 *ssid, size_t ssid_len);
	 ...
	 * (management frame processing) to wpa_supplicant.
	 */
	int (*set_bssid)(void *priv, const uint8 *bssid);
	 ...
	 * (management frame processing) to wpa_supplicant.
	 */
	int (*send_mlme)(void *priv, const uint8 *data, size_t data_len);
	 ...
	 */
	 int (*mlme_add_sta)(void *priv, const uint8 *addr, const uint8 *supp_rates,
	 ...
	 * (management frame processing) to wpa_supplicant.
	 */
	 int (*mlme_remove_sta)(void *priv, const uint8 *addr);
};
