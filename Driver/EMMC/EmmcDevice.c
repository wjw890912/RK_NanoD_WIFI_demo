...
*---------------------------------------------------------------------------------------------------------------------
*/
static EMMC_DEVICE_CLASS * gpstEmmcDevISR[SDC_MAX];
...
rk_err_t EmmcDevSuspend(HDC dev);
rk_err_t EmmcDevDelete(HDC dev);
...
HDC EmmcDev_Create(uint32 DevID, void * arg)
{
    ...
    gpstEmmcDevISR[DevID] = pstEmmcDev;

#ifndef _EMMC_BOOT_
#ifdef __SYSTEM_FWANALYSIS_FWANALYSIS_C__
    FW_ModuleOverlay(MODULE_ID_EMMC_DEV, MODULE_OVERLAY_CODE);
#endif
#endif
...
rk_err_t EmmcDevDeInit(EMMC_DEVICE_CLASS * pstEmmcDev)
{
     SDC_DevReset(pstEmmcDev->GpioID, pstEmmcDev->DevResetGPIO, 1);
     return RK_SUCCESS;
}
/*******************************************************************************
** Name: EmmcDevInit
