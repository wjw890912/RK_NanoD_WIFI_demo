...
COMMON API void MediaTask_Enter(void)
{
    ...
    AudioDev_SetSampleRate(hAudio, 0, CodecFS_44100Hz);
    AudioDev_SetTrackLen(hAudio, 4096);
    ...
    while(fifoDev_Read(hFifo, Buf + Bank * 4096, 4096, SYNC_MODE, NULL) == 4096)
    {
...
INIT API rk_err_t MediaTask_Init(void *pvParameters)
{
    ...
    FW_ModuleOverlay(MODULE_ID_MEDIA_TASK, MODULE_OVERLAY_ALL);
    #endif
