...
CgUpnpAvRenderer *dmr_create(char* suffix)
{
    ...
    // Create player.
    Mplayer_Init();
    player_create(&player);printf("--test--goin dmr_create4\r\n");
    cg_upnpav_dmr_setuserdata(dmr, player);printf("--test--goin dmr_create5\r\n");

    // Set state listener.
    player_state_listener(player, dmr_play_state_changed);printf("--test--goin dmr_create6\r\n");
    ...
    //printf("--test--goin dmr_create8\r\n");

    cg_upnpav_dmr_start(dmr);

    //printf("--test--goin dmr_create9\r\n");
    ALOGD("Media renderer start, %s.\r\n", cg_upnp_device_getudn(dmr->dev));
