...
****************************************/
BOOL cg_upnp_device_parsedescription(CgUpnpDevice *dev,  char *desciption, size_t descriptionLen)
{
...
BOOL cg_upnp_device_postsearchresponse(CgUpnpDevice *dev, CgUpnpSSDPPacket *ssdpPkt, const char *st, const char *usn)
{
    ...
    cg_log_info("Post search response to <%s>, ramain = %d\r\n", remoteAddr, xPortGetFreeHeapSize());
    ssdpCount = cg_upnp_ssdp_getannouncecount();
...
BOOL cg_upnp_device_start(CgUpnpDevice *dev)
{
	...
	printf("cg_http_serverlist_start_l\r\n");
#endif
    /**** SSDP Server ****/
    if (cg_upnp_ssdp_serverlist_open(dev->ssdpServerList) == FALSE)
        return FALSE;
	printf("cg_upnp_ssdp_serverlist_open_l\r\n");
    ...
    //printf("--test--gon to device annouce");
    /**** Announce ****/
