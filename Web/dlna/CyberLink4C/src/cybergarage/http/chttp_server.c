...
static void cg_http_server_thread(CgThread *thread)
{
    ...
    CgSocket *serverSock;
    CgSocket *clientSock;
    char threadname[32] = {"http_clinet"};
    int threadnum = 0;
    cg_log_debug_l4("Entering...\n");
    ...
    while (cg_thread_isrunnable(thread) == TRUE) {
        ...
        threadnum++;
        threadname[11] = threadnum+'0';
        FreeRtos_setParamter(httpClientThread,threadname,DLNA_MINIMAL_STACK_SIZE*8, DLNA_PRIORITY);
#endif
...
#if defined (FREE_RTOS)
    FreeRtos_setParamter(httpServer->acceptThread,"http_server", DLNA_MINIMAL_STACK_SIZE*8, DLNA_PRIORITY);
#endif
    /**** Thanks for Makela Aapo (10/31/05) ****/
