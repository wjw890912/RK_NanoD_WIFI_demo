 ...
 */
#if (defined __ANDROID__) || defined(FREE_RTOS)
#define LOG_TAG "CyberLink"

#if !defined(FREE_RTOS)
#include <utils/Log.h>
#endif
#define LOG_LEVEL SEV_DEBUG_L1//SEV_DEBUG_L1

void cg_log_print(int severity, const char *file, int line_n, const char *function, const char *format, ...)
{
    va_list list;
    char log_line[MAX_LOG_STRING] = { 0 };
    ...
    vsnprintf(log_line, MAX_LOG_STRING, format, list);
    //vsprintf(log_line, format, list);
    va_end(list);

    //ALOGD("%s - %s", function, log_line);
#if !defined(FREE_RTOS)
    ALOGD("%s", log_line);
#else
    printf("%s", log_line);
