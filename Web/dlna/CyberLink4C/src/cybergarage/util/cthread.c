...
static void FreeRtos_CodeProc(void const *pvparamter)
{
    CgThread *thread = (CgThread *)pvparamter;
    if (thread->action != NULL)
        thread->action(thread);
...
void FreeRtos_setParamter(void *pvparamter,char *name, unsigned short usStackDepth, unsigned portBASE_TYPE uxPriority)
{
    CgThread *thread = (CgThread *)pvparamter;
    thread->usTasckDepth = usStackDepth;
    thread->uxPriority = uxPriority;
    #ifdef _LOG_DEBUG_
    if(name != NULL)
    {
        int namelen = strlen(name);
        memset(thread->friendlyName, 0 , 32);
        memcpy(thread->friendlyName, name ,namelen);
    }
    else
    {
        memset(thread->friendlyName, 0 , 32);
...
BOOL cg_thread_delete(CgThread *thread)
{
    ...
    memcpy(&threadTemp,thread,sizeof(CgThread));

    free(thread);

    if (threadTemp.runnableFlag == TRUE)
    cg_thread_stop(&threadTemp);
}
...
****************************************/
int stak_num = 0;
unsigned char stak_buffer[20] = "dlna";
...
#elif defined(FREE_RTOS)
    if (thread->usTasckDepth == 0)
        thread->usTasckDepth = DLNA_MINIMAL_STACK_SIZE*8;
    if (thread->uxPriority == 0)
        thread->uxPriority = DLNA_PRIORITY;
#ifdef _LOG_DEBUG_
    if(thread->friendlyName[0] == 0)
    {
        stak_num++;
        stak_buffer[4] = stak_num + '0';
        xTaskCreate(FreeRtos_CodeProc, stak_buffer, thread->usTasckDepth , thread, thread->uxPriority, &(thread->pThread),NULL);
    }
    else
    {
        xTaskCreate(FreeRtos_CodeProc, thread->friendlyName, thread->usTasckDepth , thread, thread->uxPriority, &(thread->pThread),NULL);
    }
#else
    stak_num++;
    stak_buffer[4] = stak_num + '0';
...
BOOL cg_thread_stop_with_cond(CgThread *thread, void *cond)
{
#if defined (WINCE)
    int i, j;
    BOOL result;
