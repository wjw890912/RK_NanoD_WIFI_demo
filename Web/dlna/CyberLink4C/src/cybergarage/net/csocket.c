...
#if (!defined(WIN32) || defined(__CYGWIN__)) && !defined(BTRON) && !defined(ITRON) && !defined(TENGINE) && !defined(FREE_RTOS)
...
#elif defined(FREE_RTOS)
    struct  sockaddr sockClientAddr;
    socklen_t nLength = sizeof(sockClientAddr);
    cg_socket_setid(clientSock, accept(serverSock->id, &sockClientAddr, &nLength));
#else
...
cg_log_debug_s("clientSock->id = %d\n", clientSock->id);
    ...
    if (getsockname(clientSock->id, (struct sockaddr *)&sockaddr, &socklen) == 0 )  {
        strcpy(localAddr, cg_socket_getaddress(clientSock));
        sprintf(localPort, "%d", cg_socket_getport(clientSock));
    }
...
size_t cg_socket_sendto(CgSocket *sock, const char *addr, int port, const char *data, size_t dataLen)
{
    ...
    {
       printf("cg_socket_tosockaddrinfo error\n");
       return -1;
    }
...
ssize_t cg_socket_recv(CgSocket *sock, CgDatagramPacket *dgmPkt)
{
    ...
    cg_socket_datagram_packet_setlocaladdress(dgmPkt, cg_socket_getaddress(sock));
    cg_socket_datagram_packet_setremoteaddress(dgmPkt, inet_ntoa(from.sin_addr));
    cg_socket_datagram_packet_setremoteport(dgmPkt, ntohl(from.sin_port)>>16);
#else
