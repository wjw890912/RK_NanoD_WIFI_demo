...
#elif !defined(BTRON) && !defined(ITRON) && !defined(TENGINE) && !defined(FREE_RTOS)
...
int get_ipaddr(int ipnum, char *ipaddr)
{
    char iptab[4] = {0};

    iptab[3] = (uint8_t)(ipnum >> 24);
    iptab[2] = (uint8_t)(ipnum >> 16);
    iptab[1] = (uint8_t)(ipnum >> 8);
    iptab[0] = (uint8_t)(ipnum);

    sprintf(ipaddr, "%d.%d.%d.%d", iptab[0], iptab[1], iptab[2], iptab[3]);
}
...
int cg_net_gethostinterfaces(CgNetworkInterfaceList *netIfList)
{
    ...
    netIf = cg_net_interface_new();
    //cg_net_interface_setname(netIf, mNetif->name);

    get_ipaddr(mNetif->ip_addr.addr, ipaddr);
    cg_net_interface_setaddress(netIf, ipaddr);

    cg_net_interfacelist_add(netIfList, netIf);

    while (mNetif->next != NULL){
        mNetif = mNetif->next;
        netIf = cg_net_interface_new();
        //cg_net_interface_setname(netIf, mNetif->name);
...
char *cg_net_selectaddr(struct sockaddr *remoteaddr)
{
    ...
    if (mNetif->ip_addr.addr) {
        get_ipaddr(mNetif->ip_addr.addr, ipaddr);
        return cg_strdup(ipaddr);
    }
...
#elif !defined(HAVE_IFADDRS_H) || defined(TARGET_OS_IPHONE) || defined(TARGET_IPHONE_SIMULATOR)
char *cg_net_selectaddr(struct sockaddr *remoteaddr)
{
