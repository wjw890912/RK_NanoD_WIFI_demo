...
static struct netif       eth_ip_handle;
#define ETH_IP_HANDLE    &eth_ip_handle
...
struct netif* wiced_ip_handle[ 4 ] =
{
	...
	[WICED_ETHERNET_INTERFACE] = ETH_IP_HANDLE
};
...
extern err_t asix8872B_ethernetif_init( /*@partial@*/ struct netif *netif );
wiced_result_t wiced_ip_up( wiced_interface_t interface, wiced_network_config_t config, const wiced_ip_setting_t* ip_settings )
{
	...
	{
	    if ( NULL == netif_add( &IP_HANDLE(interface), &ipaddr, &netmask, &gw, (void*) WICED_TO_WWD_INTERFACE( interface ), asix8872B_ethernetif_init, ethernet_input ) )
	    {
