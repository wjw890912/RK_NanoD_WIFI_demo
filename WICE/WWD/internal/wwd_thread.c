...
#include "wwd_poll.h"
#include "wwd_rtos.h"
#include "../include/RTOS/wwd_rtos_interface.h"
#include "wwd_buffer_interface.h"
#include "../include/network/wwd_network_interface.h"
...
void wwd_thread_func( unsigned long /*@unused@*/thread_input ) /*@globals killed wwd_transceive_semaphore@*/ /*@modifies wwd_wlan_status, wwd_bus_interrupt, wwd_thread_quit_flag@*/;
...
#ifndef __WICE_WIFI_TRANS_C__//2015-7-27
...
#else
    RKTaskCreate(TASK_ID_WWD, 0, NULL);
#endif
...
int8_t wwd_thread_send_one_packet( void ) /*@modifies internalState @*/
{
    ...
    //WPRINT_WWD_DEBUG(("\nWcd:> Sending pkt 0x%08X", (unsigned int)tmp_buf_hnd ));
...
void wwd_thread_notify_irq( void )
{
    ...
    {
        //printf("give wwd_transceive_semaphore start\r\n");
        ...
        // printf("give wwd_transceive_semaphore end\r\n");
    }
}
 ...
 *
 */
void wwd_thread_func( unsigned long /*@unused@*/thread_input ) /*@globals killed wwd_transceive_semaphore@*/ /*@modifies wwd_wlan_status, wwd_bus_interrupt, wwd_thread_quit_flag, wwd_inited, wwd_thread@*/
{
    int8_t       rx_status;
    int8_t       tx_status;
    wwd_result_t result;
...
#else
     printf("wwd task del no\n");
     RKTaskDelete(TASK_ID_WWD, 0, ASYNC_MODE);
     while(1);
#endif
}
