...
fs_free(struct fs_file *file)
{
    ...
    //printf("111111111111 normal DEV_CLASS_FILE close 111111111111\n");
...
fs_open(char *name)
{
   ...
   // printf("11111111 DEV_CLASS_FILE open ok 11111111\n");
    stFileAttr.Path = L"C:\\";
    if((name[0] == '/') &&  (name[1] == 0))
    {
    ...
    file->hFile = FileDev_OpenFile(hFileDev, NULL, READ_ONLY, &stFileAttr);
    if((rk_err_t)file->hFile <= 0)
    {
        rk_print_string("File Open Failure\n");
        goto err;
    }
    //printf("222222222 hFileDev open ok 2222222222\n");
...
err:
    RKDev_Close(hFileDev);
    //printf("11111111111111 err DEV_CLASS_FILE close 11111111111111\n");
    rkos_memory_free(file);
    return NULL;
}
