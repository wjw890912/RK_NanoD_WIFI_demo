...
extern int32_t x509parse_dn_gets( char *buf, const char *end, const x509_name *dn );
...
tls_result_t tls_host_receive_packet(ssl_context* ssl, tls_packet_t** packet, uint32_t timeout)
{
    ...
    switch(ssl->transport_protocol)
    {
        case TLS_TCP_TRANSPORT:
            result = network_tcp_receive( (wiced_tcp_socket_t*) ssl->receive_context, (wiced_packet_t**) packet, timeout );
            break;

        case TLS_EAP_TRANSPORT:
#ifndef DISABLE_EAP_TLS
            result = supplicant_receive_eap_tls_packet( ssl->receive_context, packet, timeout );
            break;
#endif /* DISABLE_EAP_TLS */
